...
export type DiscoveryDecider = {
  decide(input: { goal: string; observation: BrowserObservation; stepNumber: number; outputs: Record<string, string> }): Promise<DiscoveryAction>;
};
...
export function createOpenAIDecider(options: { apiKey?: string; model: string }): DiscoveryDecider {
  ...
  return {
    async decide(input) {
      ...
      return DiscoveryActionSchema.parse(JSON.parse(response.output_text));
    }
  };
}
